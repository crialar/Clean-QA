...
(xbench-style deterministic QA with 35 checks across 5 groups — 32
active by default, plus 3 opt-in checks) for validating
any TMX/MQXLIFF file (or a monolingual Word .docx) before delivery.
...
Double-click "Clean&QA.exe" or "script_requirements.bat"

This will install all required Python packages
Only needed the first time (may take 5-10 minutes)
Double-click "Clean&QA.exe" or "script_Clean&QA.bat" to launch the application
...
-----------------------
1. Double-click "Clean&QA.exe" or "script_Clean&QA.bat"
...
2. Either upload a TMX/MQXLIFF (or a monolingual Word .docx) directly, or
   click "Use anonymized output" to QA the file you just generated in the main tab.
...
Note: for a monolingual Word .docx upload, only the HTML report is
offered (no cleaned-file output) and only single-text checks run —
you fix the document yourself in Word from the report.

The QA tab runs 35 checks across 5 groups (Content, Terminology,
Tags, Numeric Elements, Spelling). 32 are active by default; 3
ship opt-IN — confusable_pairs (real-word spelling traps that
spell-check can't catch) plus two custom-regex checks.
You provide the word groups via the textarea or an .xlsx upload —
no built-in language packs are shipped. Spell-check (Hunspell) uses the
bundled LibreOffice dictionaries for en, es, fr, de and it, and
auto-downloads 33 more languages on first detection; misspellings
appear with a red wavy underline and a per-run ignore-words list
controls noise. Untick any category in ⚙️ Configuration if you
don't need it. See USER_GUIDE.txt for details.
...
INCLUDED FILES
--------------
- Clean&QA.exe            : Application launcher
- script_Clean&QA.bat     : Application launcher
