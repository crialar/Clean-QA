...
class MQXLIFFAnonymizer:
    ...
    def anonymize_with_presidio(self, text: str, lang: str = "en") -> str:
        ...
        try:
            ...
            # Previously swallowed silently — for clinical PII that's
            # dangerous (a Presidio crash would let the original text
            # through with zero user feedback). Log to stderr AND mark the
            # failure on the instance so the caller can surface a warning
            # in the UI ("PII layer failed on N segments").
            import logging
            logging.getLogger(__name__).warning(
                "Presidio PII layer failed on a segment (lang=%s): %s",
                lang, exc,
            )
            self.stats["presidio_failures"] = self.stats.get("presidio_failures", 0) + 1
    ...
    def process_text_node(self, text: str, 
        ...
        if use_safe_regex:
            ...
            # term embedded in an address on its own, shredding the email/URL and
            # leaking the rest (and stopping a later layer like Presidio from
            # catching the now-broken fragment). Atomically redact only the
    ...
    def anonymize_mqxliff(self, xml_content: bytes, 
        ...
        # preview pass did not, so previews of EN→DE / EN→FR / etc. ran
        # Presidio with the wrong language and showed a different result
