...
def get_highlight_boundary(category_id: Optional[str]) -> Tuple[str, str]:
    ...
    # Task #65 / #79 / #67 — double_spaces tokens are a run of 2+ space
    # characters: ASCII space (U+0020) or non-breaking space (U+00A0),
    # including a mix of the two (see _DOUBLE_SPACE_CHARS). The default word
...
# (U+0020) and non-breaking space (U+00A0) only — the two whitespace
# characters memoQ actually renders with width in its editor, so a run of 2+
# (including a mix of the two) is a redundant double the reviewer can see and
# fix. The exotic Unicode space separators (narrow no-break U+202F, thin
# U+2009, hair, figure/punctuation/en/em, ideographic U+3000, U+1680, U+205F)
# are deliberately excluded: memoQ does not display them as visible doubles, so
# flagging them produced warnings the reviewer could neither see nor act on.
# Tab, newline and zero-width characters are excluded for the same reason
# (stored as inline tags or rendered with no width). This keeps double_spaces
# consistent with the whitespace_edges policy (_FLAGGABLE_EDGE_WS).
_DOUBLE_SPACE_CHARS = "\u0020\u00a0"
...
def dotify_double_space(matched: str) -> str:
    ...
    The ``double_spaces`` token is a run of 2+ space characters — ASCII space
    (U+0020) or non-breaking space (U+00A0), see ``_DOUBLE_SPACE_CHARS``. HTML
    ...
    sees exactly how many (and that a non-breaking space is hiding there).
    Shared by the in-app QA panel (app.py) and the exported HTML report so both
    renderers stay in lock-step. Any non-space character (which should never
    appear in a double_spaces token) is passed through untouched.
    """
    return "".join(
    ...
    if fmt == "html":
        ...
        def _highlight(text: str, span, category_id: Optional[str] = None) -> str:
            ...
            def _wrap(matched: str, *, underline: bool) -> str:
                ...
                # of 2+ space characters (ASCII U+0020 *or* a non-breaking
                # space U+00A0). HTML collapses consecutive
