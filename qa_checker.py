...
def _check_whitespace_edges(seg, cfg, ctx):
    ...
    ``target_text`` is the tag-free reconstruction, and ``_element_text``
    never fabricates whitespace at the very start/end from an adjacent inline
    tag (it only inserts a separator *between* two glued non-space runs), so a
    flagged edge space is always genuine segment content — never an artifact
    of a tag sitting at the segment boundary."""
    tgt = seg["target_text"]
    if not tgt:
        return []
    side = []
