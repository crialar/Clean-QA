...
HIGH_PRIORITY_RANK: Dict[str, int] = {
    ...
    "polarity_mismatch": 2,      # Task #71 — negation-count drift, content coherence
...
CATEGORY_METADATA: Dict[str, Dict[str, Any]] = {
    ...
    "polarity_mismatch":          {"label": "Polarity mismatch (negation count)",    "icon": "🧩", "group": "Content",          "severity": SEVERITY_HIGH},
...
def _element_text(element) -> str:
    ...
    def visit(el, crossed_inline: bool = False):
        if el.text:
            _append_text_with_boundary(parts, el.text, crossed_inline)
        for child in el:
            tag = etree.QName(child.tag).localname
            is_inline = tag in _INLINE_TAG_NAMES_FOR_BOUNDARY
            # Recurse so paired wraps (g/mrk/sub) and any other nested
            # text content is captured. The flag tells the recursive call
            # that the first text it appends crossed an inline-tag
            # boundary relative to whatever is currently last in *parts*.
            visit(child, crossed_inline=is_inline)
            if child.tail:
                _append_text_with_boundary(parts, child.tail, is_inline)
...
def _filter_content_tags(tags):
    ...
    return [(name, tid) for name, tid in tags
            if name not in _ANNOTATION_TAG_NAMES]
...
# --- Task #71: polarity (negation count) -----------------------------------
...
def _check_polarity_mismatch(seg, cfg, ctx):
    """Flag when source and target have a different count of negation
    tokens — a strong heuristic for accidental affirmation→negation
    swaps (or vice versa).
    ...
    single end-of-run notice. The check is a count comparison only —
    it does NOT claim to detect *which* negation flipped; reviewers
    inspect the segment manually.
    """
    ...
    return [_make_issue(
        ...
        f"Negation count differs (source: {src_count}, target: {tgt_count}). "
