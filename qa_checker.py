...
def _check_whitespace_edges(seg, cfg, ctx):
    ...
    The edge is judged on the marker-rendered ``target_text_display`` (inline
    tags shown as ``[id]`` / ``[id]…[/id]``), NOT on the tag-free
    ``target_text``. memoQ renders special whitespace such as a tab as an
    inline tag (a brown "tab" chip). When such a tag sits at the segment edge
    with a real space next to it (e.g. a leading tab chip then a space), the
    tag-free reconstruction drops the chip and the space lands at position 0,
    looking like a leading edge space even though it is really *internal*
    whitespace between the tag and the first word — memoQ reports zero errors
    for it. Judging on the display keeps the `[id]` marker at the edge, so the
    adjacent space is no longer treated as an edge space. A genuine stray edge
    space with no tag shielding it (display also starts/ends with the space)
    still fires. Falls back to ``target_text`` when no display is supplied."""
...
def _tag_pairing_broken(tags):
    ...
    for pos, (name, tid) in enumerate(tags):
