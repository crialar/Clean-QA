...
def _load_dictionary_from_disk(lang: str):
    ...
    try:
        ...
        _NEGATIVE_CACHE[lang] = f"spylls failed to load: {type(exc).__name__}: {exc!r}"
        # Dump full traceback next to the bundled dict so users on Windows
        # can share the exact failure point. Best-effort: ignore any IO error.
        try:
