...
def _qa_highlight(text: str, span=None, category_id: str = None) -> str:
    ...
    # Task #65 — double_spaces highlight: consecutive spaces collapse in
    # HTML, so the painted span was invisible. Render the matched payload
    # with each space (ASCII *or* a non-breaking / narrow / thin / other
    # Unicode space variant) swapped for a middle dot `·` so the reviewer
    # sees exactly where (and how many) extra spaces live in the target.
    # See qa_checker.dotify_double_space (shared with the HTML report).
    def _format_match(matched: str) -> str:
        if category_id == "double_spaces":
            return qa_checker.dotify_double_space(matched)
        return matched
