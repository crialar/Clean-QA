...
def _render_qa_check_tab():
    ...
    with st.expander("⚙️ Configuration", expanded=False):
        ...
        with cap_col:
            st.markdown(
                "**35 checks available / 32 active by default.** "
        ...
        with tick_col:
            if st.button("✓ Defaults", key="qa_tick_all_btn",
                         help=("Restore the 32 default-ON checks. Leaves the "
...
def _detect_similar_source_variants(segments: list, source_threshold: int = 85,
    ...
    if not segments:
        return groups
    if len(segments) > LARGE_FUZZY_NOTICE_THRESHOLD:
        st.info(
            f"ℹ️ Large file ({len(segments):,} unique-source segments) with "
            "variant matching enabled — this analysis may take a while. The "
            "app isn't frozen; please wait."
        )
    ...
    for i, seg_a in enumerate(segments):
        ...
        for j in range(i + 1, len(segments)):
            ...
            if seg_b["key"] in used:
                continue
            sratio = difflib.SequenceMatcher(None, seg_a["source"], seg_b["source"]).ratio()
            if sratio >= thr:
                members.append(dict(seg_b, _similarity=sratio * 100))
        if len(members) < 2:
            continue
        if tmax < 1.0:
            differs = any(
...
(<b>MQXLIFF</b>) and translation memories (<b>TMX</b>). The original XML structure and
...
<div style="display:grid; grid-template-columns:1fr 1fr; gap:0.55rem; margin-bottom:1.4rem;">
  ...
  <div style="border:1px solid #bcbdbe; border-radius:8px; padding:0.65rem 0.85rem;">
    <b style="color:#134277;">🛡️ QA Check</b><br><span style="color:#5e5f6b;">Run 35 quality checks on any file, fully independent from the anonymization pipeline.</span></div>
...
<p style="color:#5e5f6b; line-height:1.55; margin:0 0 0.7rem 0;">
  Five layers run in sequence; each protects what the previous one already redacted, so nothing is
...
<div style="background:#f7f3fb; border-left:4px solid #6c3483; border-radius:8px; padding:1rem 1.2rem; margin-bottom:1.4rem;">
  ...
  <p style="margin:0 0 0.7rem 0; color:#130e45; line-height:1.6;">
    ...
    corrected file plus a CSV and an interactive <b>HTML report</b>. Spelling is opt-in and
...
def main():
    ...
    with st.container(key="app_header"):
        ...
        <p style="color: rgba(255,255,255,0.95) !important; margin: 0.5rem 0 0 0; font-size: 1.1rem; font-weight: 400; letter-spacing: 1px;">
            Anonymize, clean &amp; run QA on bilingual memoQ documents
        </p>
            """, unsafe_allow_html=True)
        with _help_col:
    ...
    with st.sidebar:
        ...
        if dedup_tmx:
            variant_source_threshold = st.slider("Variant source similarity (≥%)", 50, 100, 90, 5, help="Only affects the 'Similar source' section. How alike two sources must be to count as variants. The Identical and Same-source sections always use exact matching.")
            variant_target_max = st.slider("Variant max. target similarity (≤%)", 50, 100, 90, 5, help="Secondary filter for the 'Similar source' section. Lower it to show only variants whose translations also differ. 100% = show all.")
        else:
